...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
bool RakPeer::Connect( const char* host, unsigned short remotePort, char* passwordData, int passwordDataLength, void* pProxy )
{
	// If endThreads is true here you didn't call Initialize() first.
	if ( host == 0 || endThreads || connectionSocket == INVALID_SOCKET )
		return false;

	pRakPeerProxy = pProxy;
...
//		return;
	...
	SocketLayer::Instance()->SendTo( connectionSocket, (const char*)bitStream.GetData(), bitStream.GetNumberOfBytesUsed(), ( char* ) host, remotePort, this->pRakPeerProxy );
...
#endif
//#pragma once
//#include "../CAlphaBot.hpp"
//static std::mutex mtx2000;
bool RakPeer::HandleRPCPacket( const char *data, int length, PlayerID playerId )
{
	...
	/*if (RPCHandleCallbackFunc)
		RPCHandleCallbackFunc(this->botID, (__int32)rpcID, bs2, this);//����� ��������� �����, � ������� ����� ������ ���������� ��� ��� �������
															 //���� ��������*/
	if (handler_rpc) {
...
#endif
{
	...
	if (rakPeer->IsBanned( rakPeer->PlayerIDToDottedIP( playerId ) ))
	{
		...
		SocketLayer::Instance()->SendTo( rakPeer->connectionSocket, (char*)&c, 2, playerId.binaryAddress, playerId.port, rakPeer->pRakPeerProxy );
...
//		char data[MAX_OFFLINE_DATA_LENGTH];
	...
	else if ((unsigned char)(data)[0] == ID_OPEN_CONNECTION_COOKIE && length == sizeof(unsigned char)*2+1) // 0.3.7 RAKSAMP HACK PONPON
	{
		...
		*(WORD*)&c[1] = (*(WORD*)&data[1] ^ /*NETCODE_CONNCOOKIELULZ*/0x6969);
		...
		SocketLayer::Instance()->SendTo( rakPeer->connectionSocket, (char*)&c, 3, playerId.binaryAddress, playerId.port, rakPeer->pRakPeerProxy );
		return;
	}
	...
	else
	{
			...
			|| (unsigned char)(data)[0] == ID_PING)	&& length == sizeof(unsigned char)+sizeof(RakNetTime) )
		{
				...
				rakPeer->AllowIncomingConnections() ) // Open connections with players
			{
				...
				//outBitStream.Write<unsigned __int32>(5444454);
				...
				SocketLayer::Instance()->SendTo( rakPeer->connectionSocket, (const char*)outBitStream.GetData(), outBitStream.GetNumberOfBytesUsed(), (char*)rakPeer->PlayerIDToDottedIP(playerId) , playerId.port, rakPeer->pRakPeerProxy );
#endif
			}
		}
...
//	unsigned numberOfBytesUsed;
	...
	{
		// Read a packet
		gotData = SocketLayer::Instance()->RecvFrom( connectionSocket, this, &errorCode, pRakPeerProxy );
	...
	while (rcs)
	{
		...
		if (rcs->nextRequestTime < timeMS)
		{
			...
			rcs->requestsMade++;
			rcs->nextRequestTime=timeMS+1000;//1000
			...
			//client
			SocketLayer::Instance()->SendTo( connectionSocket, (char*)&c, 3, rcs->playerId.binaryAddress, rcs->playerId.port, this->pRakPeerProxy );
		}
	...
	//	remoteSystemList[ remoteSystemIndex ].allowPlayerIdAssigment=true;
		if ( remoteSystemList[ remoteSystemIndex ].isActive )
		{
			...
			remoteSystem->reliabilityLayer.Update( connectionSocket, playerId, MTUSize, timeNS, messageHandlerList, this->pRakPeerProxy ); // playerId only used for the internet simulator test
...
#pragma warning(disable:6387) // drop warning timerHanlde == 0, because assert( timerHandle );
