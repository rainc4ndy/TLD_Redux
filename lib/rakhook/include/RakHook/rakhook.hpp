...
#include <BitStream.h>
#include "RakClientInterface_Orig.h"
...
namespace rakhook {
    ...
    using send_t = bool(RakNet::BitStream *bs, PacketPriority &priority, PacketReliability &reliability,
                        char &ord_channel);
    using receive_t = bool(Packet *packet);
    using send_rpc_t = bool(int &id, RakNet::BitStream *bs, PacketPriority &priority, PacketReliability &reliability,
                            char &ord_channel, bool &sh_timestamp);
    using receive_rpc_t = bool(unsigned char &id, RakNet::BitStream *bs);

    inline bool initialized = false;
    inline RakClientInterface_Orig *orig = nullptr;
    ...
    bool send_rpc(int id, RakNet::BitStream *bs, PacketPriority priority, PacketReliability reliability,
                  char ord_channel, bool sh_timestamp);
