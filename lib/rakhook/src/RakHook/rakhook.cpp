...
bool rakhook::initialize() {
    ...
    auto **rakclient_interface = std::bit_cast<RakClientInterface_Orig **>(samp_info + offsets::rakclient_interface());
    if (!*rakclient_interface)
        return false;

    orig                 = *rakclient_interface;
    hooked_interface     = new hooked_rakclient_interface(orig);
    *rakclient_interface = std::bit_cast<RakClientInterface_Orig *>(hooked_interface);
