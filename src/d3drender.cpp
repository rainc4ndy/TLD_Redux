...
void CD3DRender::D3DLine(int x, int y, int x2, int y2, D3DCOLOR color, float thickness)
{
	if (SUCCEEDED(Begin(D3DPT_TRIANGLESTRIP))) // Using triangle strip to create a thick line
	{
		...
		float offsetX = -dy * (thickness / 2);
		float offsetY = dx * (thickness / 2);

		// First point (x, y) adjusted for thickness
		D3DColor(color);
		D3DVertex2f(x + offsetX, y + offsetY); // First vertex of the quad
		D3DVertex2f(x - offsetX, y - offsetY); // Second vertex of the quad

		// Second point (x2, y2) adjusted for thickness
		D3DVertex2f(x2 + offsetX, y2 + offsetY); // Third vertex of the quad
		D3DVertex2f(x2 - offsetX, y2 - offsetY); // Fourth vertex of the quad

		End();
	}
}

void CD3DRender::D3DCircle(int centerX, int centerY, float radius, D3DCOLOR color, int numSegments)
{
	if (SUCCEEDED(Begin(D3DPT_LINESTRIP)))
	{
		...
		{
			float theta = (2.0f * D3DX_PI * i) / numSegments;  // Current angle
			float x = centerX + radius * cosf(theta);          // X coordinate
			float y = centerY + radius * sinf(theta);          // Y coordinate
